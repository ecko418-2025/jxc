...
为了满足企业级的合规溯源需求，系统在 `v1.1` 引入了全方位的**系统安全日志（Audit Logs）** 机制。

### 7.1 后端拦截器
位于 `functions/api/index.js` 中的底层云函数，内置了一个全局的请求拦截器，对除了 `get*` 之外的所有操作（增、删、改、确认等）自动落库记录：
- 自动提取操作动作类型（如 `createSalesOrder`、`adjustInventoryStock`）。
- 自动关联业务单据号、商品 ID、操作人时间戳，存入 `audit_logs` 数据库表。

### 7.2 前端可视化与全局搜索
在【系统设置】页面（`Settings/index.tsx`），提供了全量日志的展示面板，支持：
1. **自动翻译与清洗**：能够将系统底层难懂的 UUID 自动转化显示为您熟悉的 `[业务单据编号]` 或 `[商品编号+名称]`。
2. **全局智能匹配**：内置的搜索框不仅仅支持搜索中文动作，更支持直接通过实际的单据编号（例如 `PO2506`、`SO2506` 等）或关键字进行秒级检索，方便溯源防串。
